...
while True:
    ...
    matches = list(re.finditer(pattern, usr))
    if len(matches) > 0:
        pass
    for match in matches:
        print(match.start())
        print(match.end())
        print(usr[match.start():match.end()])
    pos = usr.find(' at ')
    if pos != -1:
        pos += 3
        flag = True
...
main_str = "hello world, hello everyone"
sub_str = "hello"
pattern = re.compile(sub_str)
matches = pattern.finditer(main_str)

for match in matches:
    print(f"Found '{sub_str}' at index {match.start()}"
...
# import os
#os.remove("G:\Memory\Python Programs\disk_link\original\storage.txt")
...
with open("F:\Memory\Python Programs\disk_link\originalnew\\appls.json", "w") as f1:
    json.dump({"chrome":"\Memory\Softwares\Chrome\Application\chrome.exe",
                      "pcl":"\Memory\Games\Minecraft\Plain Craft Launcher 2.exe"}, f1)